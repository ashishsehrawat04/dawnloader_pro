<div class="container mx-auto px-4 py-8 max-w-[1280px]">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-x-4 gap-y-8">
        ...
        @forelse($videos as $video)
            <div class="flex flex-col group">
                ...
                <div class="relative w-full aspect-video bg-gray-900 rounded-xl overflow-hidden shadow-sm group-hover:shadow-md transition-all duration-200">
                    @php
                        ...
                        if (str_contains($videoUrl, 'youtube.com') || str_contains($videoUrl, 'youtu.be')) {
                            ...
                            if (isset($urlParts['query'])) {
                                ...
                                // Using traditional PHP string functions for compatibility
                                $path = explode('/', parse_url($videoUrl, PHP_URL_PATH));
                                $videoId = end($path);
                    ...
                    <span class="absolute bottom-1 right-1 bg-black/80 text-white text-[10px] font-medium px-1.5 py-0.5 rounded-sm pointer-events-none">
                ...
                <div class="mt-3 flex items-start gap-3">
                    <div class="flex flex-col overflow-hidden">
                        <h3 class="text-base font-medium text-gray-900 line-clamp-2 leading-tight group-hover:text-indigo-600 transition-colors">
                            <a href="{{ $video->source_url }}" target="_blank" title="{{ $video->title }}">
                                {{ $video->title }}
                            </a>
                        </h3>

                        <div class="text-sm text-gray-600 mt-1 flex flex-wrap items-center gap-1">
                            <span>{{ $video->created_at->diffForHumans(null, true) }} ago</span>

                            <span class="text-gray-400">•</span>
                            <a href="{{ $video->source_url }}" target="_blank" class="text-indigo-600 hover:text-indigo-800 text-xs font-medium uppercase">
                                Open Source
                            </a>
                        </div>
                    </div>
        ...
        @empty
            <div class="col-span-full flex flex-col items-center justify-center py-32 text-gray-500">
                ...
                <p class="text-xl font-medium text-gray-600">No videos found right now.</p>
    ...
    <div class="mt-12 py-4 border-t border-gray-100 flex justify-center">
